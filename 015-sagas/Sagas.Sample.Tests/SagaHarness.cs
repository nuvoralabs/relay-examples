...
/// <summary>
/// A tiny in-memory harness that drives a saga through the real <see cref="SagaCoordinator"/> with fake
/// persistence, scheduler and bus — no database. This is exactly how the framework unit-tests sagas, and
/// it lets you assert a saga's behaviour (state, timeouts, completion) deterministically. In production
/// you replace the fakes with <c>AddRelaySagas()</c> + <c>AddRelaySagasEfCore()</c> (PostgreSQL).
/// </summary>
...
internal sealed class FakeSagaRepository : ISagaRepository
{
    ...
    public void Update(SagaRecord saga) { /* the same reference is already in the store */ }
}
...
internal sealed class FakeIntegrationBus : IIntegrationEventBus
{
    ...
    public Task Publish<TEvent>(TEvent integrationEvent, CancellationToken ct) where TEvent : IIntegrationEvent
    {
        Published.Add(integrationEvent);
        return Task.CompletedTask;
    }
    public Task PublishMany<TEvent>(IEnumerable<TEvent> integrationEvents, CancellationToken ct) where TEvent : IIntegrationEvent
        => Task.CompletedTask;
